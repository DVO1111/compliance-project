  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={onNew}
          className="flex items-center gap-2 px-4 py-2 bg-[var(--color-accent)] text-white rounded-lg text-sm font-medium hover:bg-[var(--color-accent-hover)] transition-colors"
        >
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-[var(--color-surface,#fff)] border border-[var(--color-border,#e5e7eb)] rounded-xl">
          ...
          <button onClick={onNew} className="px-4 py-2 bg-[var(--color-accent)] text-white rounded-lg text-sm font-medium">
            Get started
          </button>
        </div>
      ) : (
  ...
  if (!data) {
    return (
      <div className="p-6 max-w-5xl mx-auto text-center py-20">
        ...
        <button onClick={onBack} className="mt-4 text-sm text-[var(--color-accent)] hover:underline">Back to list</button>
      </div>
    );
  }
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {(data.status === 'draft' || data.status === 'ready') && (
            <button
              ...
              disabled={assembling}
              className="flex items-center gap-2 px-4 py-2 bg-[var(--color-accent)] text-white rounded-lg text-sm font-medium hover:bg-[var(--color-accent-hover)] disabled:opacity-50 transition-colors"
            >
      ...
      {data.status === 'ready' && (
        <>
          {/* AI Summary */}
          {data.ai_summary && (
            <div className="bg-gradient-to-br from-[var(--color-accent-soft)] to-[var(--color-accent-soft)] border border-[var(--color-accent-soft)] rounded-xl p-5">
      ...
      {data.status === 'draft' && (
        <div className="text-center py-16 bg-[var(--color-surface,#fff)] border border-dashed border-[var(--color-border,#e5e7eb)] rounded-xl">
          ...
          <button
            onClick={handleAssemble}
            className="flex items-center gap-2 px-5 py-2.5 bg-[var(--color-accent)] text-white rounded-lg text-sm font-medium mx-auto hover:bg-[var(--color-accent-hover)] transition-colors"
          >
      ...
      <div className="bg-[var(--color-surface,#fff)] rounded-2xl shadow-xl max-w-lg w-full">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              {([['scheduled', 'Scheduled', 'Full AI evidence assembly'], ['unannounced', 'Unannounced', 'Instant priority documents']] as const).map(([id, label, hint]) => (
                ...
                >
                  <span className="flex items-center gap-1.5 text-sm font-semibold text-[var(--color-text-primary,#111)]">
                    {id === 'unannounced' ? <Zap className="w-3.5 h-3.5 text-amber-500" /> : <Calendar className="w-3.5 h-3.5 text-[var(--color-accent)]" />}
                    {label}
                  </span>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" className="px-4 py-2 bg-[var(--color-accent)] text-white rounded-lg text-sm font-medium hover:bg-[var(--color-accent-hover)] transition-colors">
              Create session
            </button>
          </div>
        </form>
