...
export async function addVendor(companyId: string, v: { vendor_name: string; vendor_type: VendorType; contact_email?: string; compliance_certified?: boolean; certification_expiry?: string; risk_tier?: RiskTier }): Promise<VendorProfile | null> {
...
export async function addAudit(vendorId: string, a: { audit_type: string; findings?: string; score: number; auditor: string; next_audit_due?: string }): Promise<boolean> {
...
export async function updateVendorStatus(vendorId: string, status: string): Promise<boolean> {
